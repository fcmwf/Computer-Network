...
class GBNSender(Device, OutMixIn):
    ...
    def run(self, env: Environment):
        ...
        """
        while self.seqno < self.window_size :
            wait_pkt = self.new_packet(self.seqno ,self.message[self.absno])
            self.send_packet(wait_pkt)
            self.outbound.append(wait_pkt)
    ...
    def put(self, packet: Packet):
        ...
        if len(self.outbound) < self.window_size  and self.absno  <  len(self.message):
            wait_pkt = self.new_packet(self.seqno , self.message[self.absno])
            self.send_packet(wait_pkt)
            self.outbound.append(wait_pkt)
            self.absno = self.absno + 1
    ...
    def timeout_callback(self):
        self.dprint("timeout")
        for ele in self.outbound:
            wait_pkt = self.new_packet( ele.packet_id , ele.payload)
            self.send_packet(wait_pkt)
